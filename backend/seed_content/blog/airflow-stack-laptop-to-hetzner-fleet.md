...
Local-dev auth is deliberately loose: `SimpleAuthManager` with `SIMPLE_AUTH_MANAGER_ALL_ADMINS='true'`, a placeholder JWT secret, and `airflow:airflow` Postgres credentials. The compose file says, in a comment, *do not copy this block to anything reachable from a network*. The rest of this post is what changes when you do reach a network.
