...
**Workers stay generic; jobs live in images.** The heavy task code runs as sibling containers pulled fresh from a registry at task start. Shipping a new version of a processing stage means pushing an image, not redeploying Airflow — which, for a research pipeline, means the method can change without touching the orchestrator. The price is that workers need the Docker socket, a security trade-off to make consciously.
