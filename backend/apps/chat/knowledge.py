...
CHUNKS = [
    ...
    },
    {
        ...
        "content": (
            ...
            "linkedin.com/in/ian-ronk-7b054a120. GitHub: github.com/IANRONK. "
...
SYSTEM_PROMPT = """You are an AI assistant for Ian Ronk's personal website at ianronk.com. \
...
def search_kb(query: str, limit: int = 4) -> list[str]:
    ...
    try:
        rows = conn.execute(
            "SELECT content FROM chunks WHERE chunks MATCH ? ORDER BY rank LIMIT ?",
            (query, limit),
        ).fetchall()
